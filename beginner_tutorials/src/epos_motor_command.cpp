...
#include <sys/times.h>
#include <sys/time.h>
#include <beginner_tutorials/epos.h>

unsigned char flag = 0;
beginner_tutorials::epos tendon_speed;
...
// Create new code in the following part

void get_speed(const beginner_tutorials::epos::ConstPtr& msg)
{
  tendon_speed = *msg;
  cout << "received" << endl;
  flag = 1;
...
// void get_position(const beginner_tutorials::epos::ConstPtr& msg)
// {
//   = *msg;
//   cout << "received" << endl;
//   flag = 1;
...
int PositionControl(unsigned int* p_pErrorCode, long position)
{
	int lResult = MMC_SUCCESS;
	unsigned int lErrorCode = 0;

	lResult = ModifiedProfilePositionMode(g_pKeyHandle, g_usNodeId, lErrorCode, position);
...
int VelocityControl(unsigned int* p_pErrorCode, long velocity)
{
	int lResult = MMC_SUCCESS;
	unsigned int lErrorCode = 0;

	lResult = ModifiedProfileVelocityMode(g_pKeyHandle, g_usNodeId, lErrorCode, velocity);

	if(lResult != MMC_SUCCESS)
	{
		LogError("ModifiedProfileVelocityMode", lResult, lErrorCode);
	}
...
int main(int argc, char** argv)
{
	...
	long motor_position = 1;
	ros::init(argc,argv, "epos_motor_command");
    ros::NodeHandle n;
	...
	ros::Subscriber sub = n.subscribe("rpm", 100, get_speed);
	...
	while(ros::ok())
	{
		...
		if(flag==1)
		{
			if(tendon_speed.mode == 0)
			{
				motor_velocity = (long)(tendon_speed.velocity);
				cout << motor_velocity << endl;
			}
			else
			{
				motor_position = (long)(tendon_speed.position);
				cout << motor_position << endl;
			}
		}
		if((lResult = ParseArguments(argc, argv))!=MMC_SUCCESS)
		{
		...
		if(flag==1)
		{
			if(tendon_speed.mode == 0)
			{	
				if((lResult = VelocityControl(&ulErrorCode, motor_velocity))!=MMC_SUCCESS)
				{
					//LogError("Demo", lResult, ulErrorCode);
					return lResult;
				}
			...
			{
				if((lResult = PositionControl(&ulErrorCode, motor_position))!=MMC_SUCCESS)
				{
					//LogError("Demo", lResult, ulErrorCode);
					return lResult;
				}
